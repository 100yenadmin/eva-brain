...
- registers OpenClaw workspace docs as source `workspace-docs` when present.
  The updater first uses `EVA_BRAIN_WORKSPACE_DOCS_DIR`, then
  `agents.defaults.workspace/docs` from OpenClaw config, then
...
Before initializing the brain, verify provider reachability:

```bash
gbrain providers list
gbrain providers explain
gbrain providers test --model voyage:voyage-4-large  # pre-init key/model reachability check
```

After initializing or updating an existing Eva Brain install, verify the
configured provider path instead of forcing a model on the command line:
...
The configured-path check must report the dimensions stored in the brain config
and schema, for example `2048 dims` for the Eva Voyage profile. Without an
embedding provider, keyword search still works but semantic/hybrid retrieval
will not.
...
On local developer machines the updater resolves the docs path from
`EVA_BRAIN_WORKSPACE_DOCS_DIR` when set, otherwise from
`agents.defaults.workspace/docs` in `$HOME/.openclaw/openclaw.json`, and finally
from `$HOME/.openclaw/workspace/docs`. Register/import it with:
...
- **Workspace docs refresh** (every 15 min on OpenClaw VMs): run
  `gbrain import "$OPENCLAW_WORKSPACE/docs" --source-id workspace-docs --no-embed && gbrain embed --stale --source workspace-docs`, where
  `$OPENCLAW_WORKSPACE` is the configured `agents.defaults.workspace` value
  or `/root/.openclaw/workspace` on default VM installs.
