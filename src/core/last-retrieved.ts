...
const pendingRetrievalWrites = new Set<Promise<void>>();
...
/**
 * CLI/test seam: short-lived processes must not close PGLite while the
 * best-effort retrieval write-back is still using the same engine.
 */
...
export function bumpLastRetrievedAt(engine: BrainEngine, pageIds: number[]): void {
  ...
  pendingRetrievalWrites.add(write);
  write.finally(() => pendingRetrievalWrites.delete(write)).catch(() => { /* swallow */ });
}
