...
describe('CLI dispatcher routes reserved search subcommands', () => {
  test('gbrain search modes reaches the search command instead of keyword search', async () => {
    ...
    try {
      ...
      const proc = Bun.spawn(['bun', 'run', 'src/cli.ts', 'search', 'modes', '--json'], {
        cwd: repoRoot,
        env,
        stdout: 'pipe',
        stderr: 'pipe',
      ...
      expect(stderr).not.toContain('Usage: gbrain search <query>');
