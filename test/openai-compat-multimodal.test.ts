...
describe('embedMultimodal — openai-compat routing (#875)', () => {
  ...
  test('D12 — Eva default embedding_dimensions (2048) applies when not explicitly set', async () => {
    // configureGateway normalizes embedding_dimensions to DEFAULT_EMBEDDING_DIMENSIONS
    // when unset. Eva keeps Voyage 2048 as the downstream default even though
    // upstream v0.36 exposes ZeroEntropy via ze-switch. LiteLLM recipe's
    // default_dims=0 so we fall back
    ...
    configureGateway({
      ...
      // intentionally NO embedding_dimensions → falls back to Eva's 2048
      env: { LITELLM_BASE_URL: 'http://localhost:4000' },
      base_urls: { litellm: 'http://localhost:4000' },
    });
    fetchHandler = async () => okResponse(2048, 1);
    const result = await embedMultimodal([{ kind: 'image_base64', data: 'x', mime: 'image/png' }]);
    expect(result.length).toBe(1);
